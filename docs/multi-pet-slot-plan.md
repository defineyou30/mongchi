...
2. **슬롯 50cr에 생성 번들 확정?** (확정 시 서버 원장/quota+1이 슬롯의 선행 과제가 됨 — 이 순서 수용?)
3. **채팅 무료 티켓**: 유저당 공유(추천, 서버비 불변) / 펫당 지급(대화·비용 2배)
4. **산책 도감**: 가족 공동 도감(공유, 발견자 뱃지만 펫별 — 추천) / 펫별 도감(수집 동기 2배·노동 2배)
5. **동거 버프 밸런스 수치**: 방향만 승인하고 수치는 별도 밸런스 세션에서?
6. **careStreak 공유 전환**: 유저 습관 스트릭으로 공유 + 친구 페이지를 "N days together"로 변경 승인?
