...
| W | 내용 | 규모 |
|---|---|---|
| W1 | ✅ (커밋 031f483 랜드) 도메인 번들화 + 마이그레이션 v7 + 렌즈 헬퍼 + 생성 상태머신을 onboardingFlow로 격리(첫 펫 파괴 방지) | 대 ~4-6일 |
| W2 | ✅ (커밋 9034c7d 배포) 서버 펫 네임스페이스: generation_jobs/assets에 pet_id, 스토리지 avatars/{userId}/{petId}/, pet_slots 테이블 + 생성 게이트 | 중 ~2-3일 |
| W3 | ⏸ (진행분 `git stash` 라벨 `slot-w3-wip` 보관 — 출시 후 재개) 슬롯 구매 + 두 번째 펫 온보딩(기존 플로우 무파괴 재사용) + Splash 라우팅 pets 개수 기반 | 중대 ~3-5일 |
| W4 | ⬜ 미착수. 멀티펫 표면: 홈 펫 전환, 친구/채팅/설정 petId 스코프, 알림 복수형 | 대 ~4-6일 |
