...
## Appendix: Chat Live Guardrail Status (absorbed from chat-live-design.md)

The original chat-live implementation design (`chat-live-design.md`, authored
2026-07-08) has been superseded by this audit and was removed after its
current-status note was folded in here. That note, current as of local
correction on 2026-07-12:

- `0014_chat_turn_guardrails.sql` owns server-side charge decisions,
  user-global rate limiting, pre-provider-call request reservation, replay of
  completed requests, and failure recovery. The client no longer sends or
  selects a `charge` field.
- `0015_chat_message_reports.sql` and the mobile report dialog were added so
  users can flag an AI response and persist a reason through
  `report_chat_message`.
- `CHAT_LIVE_ENABLED` and the mobile public feature flag both default OFF.
- The remainder of the original design document described an earlier draft
  where the client still chose `charge`; that draft is not the implementation
  of record. This audit and `docs/engineering/current/supabase-runtime.md`
  are the current source of truth for chat-live status.
