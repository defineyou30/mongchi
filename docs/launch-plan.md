...
> 최종 갱신: 2026-07-08 · 이 문서가 제품/BM/운영의 단일 기준이다.
...
| 크레딧 Phase 1c — 클라 서버잔액 동기화 (표정 팩 로컬 차감 → 서버 선차감·응답 동기화, request_id 멱등 재시도 가드 포함) | bonusCredits는 로컬 진실 유지 | ✅ 완료 (2026-07-07) |
| 멀티펫 W1 — 세션 pets[petId] 번들화 + 마이그레이션 v7 + 렌즈 헬퍼 (UI 무변경) | 첫 펫 파괴 버그(생성 시 에셋 비우기) 제거 포함 | ✅ 완료 (2026-07-07) |
| 멀티펫 W2 — 서버 펫 네임스페이스: generation_jobs/assets pet_id + pet_slots 테이블 + 슬롯 RPC 3종(grant/reserve/refund) + 스토리지 펫 경로 | 배포 완료. 상세: multi-pet-slot-plan.md 2026-07-08 노트 | ✅ 완료 (2026-07-08) |
| 멀티펫 W3 — 슬롯 구매(이사 상자·초대 시트) + 두 번째 펫 온보딩 재사용 | W2 RPC 소비 | 📋 다음 |
| 멀티펫 W4 — 홈 공존 렌더·펫 전환, 친구/채팅 petId 스코프 | | 📋 예정 |
