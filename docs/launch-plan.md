...
| 크레딧 Phase 1 — 서버 원장 + 팩 IAP + 계정 연결 권유 | credit_wallets/ledger 마이그레이션 | 📋 예정 |
| 크레딧 Phase 2 — 생성 소비 서버 차감 전환 | 무료 1회 유지 | 📋 예정 |
...
- [ ] 표정 팩 서버 크레딧 원장(Phase 1)이 근본 대응 — 현재 `generate-avatar`의 requested_states dedup/상한(6개) + rate limit fail-closed 패치는 증폭·fail-open만 차단
