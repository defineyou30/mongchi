...
> 2026-07-08 재구성: 실제 랜드 순서(커밋)로 상태를 실제화했다. 2026-07-07 git init 이후 20개 커밋, vitest 1252 그린, 세션 스키마 v7, Supabase 마이그레이션 0001–0005 배포됨.
...
| Supabase 스키마 + Edge Function + 모바일 연결 | 프로젝트 생성·배포 완료 (마이그레이션 0001–0005) | ✅ 완료·배포됨 |
...
- [x] Supabase 프로젝트 (us-east-1) + `supabase db push` + secrets + functions deploy — ✅ 완료 (마이그레이션 0001–0005 배포됨)
