...
for (const entry of screenshots) {
  const fileRegex = new RegExp(`^ios-(.+)-${escapeRegex(entry.captureLabel)}\\.png$`);
...
if (fullDevices.length === 0) {
  failures.push("No iOS device has a complete store screenshot preset set.");
}
...
for (const [deviceSlug, entries] of fullDevices) {
  const contactSheetPath = resolve(outputDirectory, `ios-${deviceSlug}-store-contact-sheet.png`);
  ...
  if (contactSheetMtime + 1000 < latestScreenshotMtime) {
    failures.push(`${contactSheetPath} is older than at least one source store screenshot. Run npm run generate:ios-store-contact-sheet.`);
  }
}

if (failures.length > 0) {
  console.error("iOS store contact sheet validation failed:");
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log("iOS store contact sheet validation passed.");
