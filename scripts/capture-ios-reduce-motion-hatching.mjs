...
const port = Number.parseInt(process.env.TINY_PET_IOS_REDUCE_MOTION_PORT ?? "8092", 10);
const startupTimeoutMs = Number.parseInt(process.env.TINY_PET_IOS_REDUCE_MOTION_STARTUP_TIMEOUT_MS ?? "90000", 10);
const settleMs = Number.parseInt(process.env.TINY_PET_IOS_REDUCE_MOTION_SETTLE_MS ?? "10000", 10);
const supportedPresets = new Set(["hatching", "pet-reveal", "terrarium", "chat", "shop"]);
const preset = process.env.TINY_PET_IOS_REDUCE_MOTION_PRESET?.trim() || "hatching";
const label = process.env.TINY_PET_IOS_REDUCE_MOTION_LABEL ?? `reduce-motion-${preset}`;
...
assertPositiveInteger(port, "TINY_PET_IOS_REDUCE_MOTION_PORT");
assertPositiveInteger(startupTimeoutMs, "TINY_PET_IOS_REDUCE_MOTION_STARTUP_TIMEOUT_MS");
assertPositiveInteger(settleMs, "TINY_PET_IOS_REDUCE_MOTION_SETTLE_MS");

if (!supportedPresets.has(preset)) {
  console.error("TINY_PET_IOS_REDUCE_MOTION_PRESET must be hatching, pet-reveal, terrarium, chat, or shop.");
  process.exit(1);
}
...
const writeReduceMotion = (enabled) => {
  const result = runText("xcrun", [
    "simctl",
    "spawn",
    targetDevice.udid,
  ...
  if (result.status !== 0) {
    throw new Error(result.stderr.trim() || `Unable to ${enabled ? "enable" : "disable"} iOS Reduce Motion in the simulator.`);
  }
};
...
const waitForMetro = async (metroProcess, metroOutput) => {
  ...
  while (Date.now() - startedAt < startupTimeoutMs) {
    if (metroProcess.exitCode !== null) {
      throw new Error("Expo Metro exited before the reduced-motion hatching screen was ready.");
    }
...
const openPresetInSimulator = () => {
  ...
  if (result.status !== 0) {
    throw new Error(result.stderr.trim() || `Unable to open Expo URL for reduced-motion ${preset}.`);
  }
};
...
const capturePreset = () => {
  ...
  if (result.status !== 0) {
    throw new Error(result.stderr.trim() || result.stdout.trim() || `Unable to capture reduced-motion ${preset}.`);
  }

  process.stdout.write(result.stdout);
};

const previousReduceMotion = readReduceMotion();
...
try {
  ...
  if (readReduceMotion() !== true) {
    throw new Error("iOS Reduce Motion did not stay enabled in the simulator defaults domain.");
  }

  const started = startMetro();
  metroProcess = started.metroProcess;
  await waitForMetro(started.metroProcess, started.metroOutput);
  openPresetInSimulator();
  ...
  if (previousReduceMotion !== null) {
    try {
      writeReduceMotion(previousReduceMotion);
    } catch (error) {
      failed = true;
      console.error(error instanceof Error ? error.message : String(error));
    }
...
console.log(`iOS Reduce Motion ${preset} evidence captured; setting restored to ${previousReduceMotion === true ? "enabled" : "disabled"}.`);
