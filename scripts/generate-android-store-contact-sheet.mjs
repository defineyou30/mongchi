...
const requestedDeviceSlug = process.env.TINY_PET_ANDROID_STORE_CONTACT_DEVICE?.trim() || null;
const labelPrefix = process.env.TINY_PET_ANDROID_STORE_CONTACT_LABEL_PREFIX?.trim() || "";
const thumbnailWidth = Number.parseInt(process.env.TINY_PET_ANDROID_STORE_CONTACT_THUMBNAIL_WIDTH ?? "220", 10);
const columns = Number.parseInt(process.env.TINY_PET_ANDROID_STORE_CONTACT_COLUMNS ?? "3", 10);
const gap = 14;
const border = 4;
...
if (!Number.isInteger(thumbnailWidth) || thumbnailWidth < 120 || thumbnailWidth > 480) {
  console.error("TINY_PET_ANDROID_STORE_CONTACT_THUMBNAIL_WIDTH must be an integer between 120 and 480.");
  process.exit(1);
}

if (!Number.isInteger(columns) || columns < 1 || columns > 6) {
  console.error("TINY_PET_ANDROID_STORE_CONTACT_COLUMNS must be an integer between 1 and 6.");
  process.exit(1);
}
...
for (const entry of screenshots) {
  const prefixedLabel = labelPrefix ? `${labelPrefix}-${entry.captureLabel}` : entry.captureLabel;
  const fileRegex = new RegExp(`^android-(.+)-${escapeRegex(prefixedLabel)}\\.png$`);
...
if (!selectedDeviceSlug) {
  console.error("No Android store screenshots found for contact sheet generation.");
  process.exit(1);
}
...
if (!selectedFiles) {
  console.error(`No Android store screenshots found for device ${selectedDeviceSlug}.`);
  process.exit(1);
}
...
if (missingPresets.length > 0) {
  console.error(`Cannot generate Android contact sheet for ${selectedDeviceSlug}; missing preset(s): ${missingPresets.join(", ")}.`);
  process.exit(1);
}
...
const contactSheetPath = resolve(outputDirectory, `android-${selectedDeviceSlug}-${labelPrefix ? `${labelPrefix}-` : ""}store-contact-sheet.png`);
writeFileSync(contactSheetPath, PNG.sync.write(sheet));

console.log(`Generated Android store contact sheet: ${contactSheetPath}`);
